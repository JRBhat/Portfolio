"""
Visia Macbeth Colour Chart Validator — simple overlay plot.
Validates LAB colour accuracy of Visia camera captures against Macbeth ColorChecker
reference values. Renders all colours on a single shared L* plot per capture mode.
"""
import cv2
import numpy as np
import pprint
...
def main():
    ...
    for n, path in enumerate(paths, start=1):
        ...
        for k, v in color_check_standard_dict.items():
            plt.axhline(y=v[0], color=macbeth_color_dict[k], linestyle="--")
            ...
            plt.plot(n, img_color_dict[k][0], "o", color=macbeth_color_dict[k])
        print()
        print(path)
