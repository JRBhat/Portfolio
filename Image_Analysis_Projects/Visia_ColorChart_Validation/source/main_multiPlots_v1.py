"""
Visia Macbeth Colour Chart Validator — individual per-colour subplots.
Validates LAB colour accuracy of Visia camera captures against Macbeth ColorChecker
reference values. Renders one subplot per colour with data points per capture mode.
"""
import cv2
import numpy as np
import pprint
...
############# prepare bounding box coordinates for each color location on color chart
...
def main():
    ...
    for n, path in enumerate(paths, start=1):
        ...
        for k, v in color_check_standard_dict.items():
            ...
            for k, v in color_chart_coord_dict.items():
                draw_rect(MaskedImg, v)
                cv2.imshow('Masked Image', MaskedImg)
                cv2.waitKey(0)
                cv2.destroyAllWindows()

            axs[count].axhline(y=v[0], color=macbeth_color_dict[k], linestyle="--")
            axs[count].plot(n, img_color_dict[k][0], "o", color=macbeth_color_dict[k])
            axs[count].set_title(k)
            axs[count].set_autoscaley_on(True)
            count += 1
        print()
        print(path)
