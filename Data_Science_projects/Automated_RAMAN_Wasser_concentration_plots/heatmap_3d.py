...
def make_3d_heatmap(input_path: Path, output_dir: Path) -> None:
    """
    Generate per-subject 3D water-profile heatmaps from Raman scan data.
    ...
    Side Effects
    ------------
    Writes one PNG per subject (``<subjectID>_3d.png``) and a
    ...
    log = QualityLog(
        output_dir,
        script_label="3D Water Profile Heatmap (v7)",
        input_path=input_path,
        header_notes=[
            "v7 uses actual (x_position, y_position) as the two spatial axes.",
            "Scattered scan data is interpolated to a regular NX_GRID x NY_GRID grid via griddata.",
        ],
    )
    ...
    def build_volume(subj, product, timepoint, log):
        """
        ...
        if len(sub) == 0:
            ...
            return fallback, x_g, y_g, np.zeros((0, 2)), 0, panel_warnings
        ...
        vol = np.full((N_DEPTH_LEVELS, NY_GRID, NX_GRID), np.nan)

        for d in range(DEPTH_MIN_UM, N_DEPTH_LEVELS):
        ...
        zoom_d = ND_OUT / float(N_DEPTH_LEVELS)
        vol_nd = np.clip(
            ndimage_zoom(vol, (zoom_d, 1.0, 1.0), order=3),
            VMIN, VMAX
        )

        return vol_nd, x_grid, y_grid, scan_xy, n_scans, panel_warnings
    ...
    def draw_box(ax, vol, x_grid, y_grid, scan_xy, n_scans):
        ...
        """
        nd, ny, nx = vol.shape
        D1d = np.linspace(DEPTH_MIN_UM, DEPTH_MAX_UM, nd)
        ...
        ax.set_zlim(DEPTH_MAX_UM, DEPTH_MIN_UM)
        ax.set_zticks(np.arange(DEPTH_MIN_UM, N_DEPTH_LEVELS, 10))
        ax.set_zticklabels([str(v) for v in np.arange(DEPTH_MIN_UM, N_DEPTH_LEVELS, 10)], fontsize=5)
    ...
    for subj in subjects:
        ...
        for i, (prod_code, prod_label) in enumerate(ROWS):
            for j, (tp_code, tp_label) in enumerate(COLS):
                ...
                vol, x_grid, y_grid, scan_xy, n_scans, panel_warnings = build_volume(
        ...
        out_path = Path(output_dir) / f"{subj}_3d.png"
