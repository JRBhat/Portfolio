...
def plot_with_matplotlib(ax, y_col, title, df_markers, df_plot, palette):
    ...
    for label, color in palette.items():
        ...
        if not grp.empty:
            # Sort by timestamp to avoid zigzag lines
            grp = grp.sort_values("timestamp")

            # Optional smoothing (comment out if not desired)
            # grp[y_col] = grp[y_col].rolling(window=5, min_periods=1, center=True).mean()

            # Identify continuous sections (no NaN gaps > 5 seconds)
            grp['time_diff'] = grp['timestamp'].diff().dt.total_seconds().fillna(0)
            gap_threshold = 5  # seconds — tweak depending on sampling
            grp['new_section'] = grp['time_diff'] > gap_threshold

            # Split into contiguous segments
            section_ids = grp['new_section'].cumsum()

            for _, seg in grp.groupby(section_ids):
                if len(seg) > 1:
                    ax.plot(seg['timestamp'], seg[y_col],
                            label=label if _ == 0 else None,  # only label once
                            color=color, linewidth=1.2, zorder=2)
                else:
                    ax.scatter(seg['timestamp'], seg[y_col],
...
def generate_plots(merged_file_path, window_duration_mins, downsample_size, marker_values):
    ...
    if not os.path.exists(seg_path):
        ...
        for y_col, title in variables:
            ...
            fig.suptitle(f'{title} (1 Min After Trigger)', fontsize=18)
