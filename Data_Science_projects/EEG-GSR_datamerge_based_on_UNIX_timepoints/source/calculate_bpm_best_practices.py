"""
BPM calculation from a PPG (photoplethysmography) signal.

Algorithm overview:
  1. Convert timestamps to seconds and uniformly resample the PPG trace.
  2. Apply a zero-phase Butterworth bandpass filter
     (:data:`PPG_BANDPASS_LOW_HZ` – :data:`PPG_BANDPASS_HIGH_HZ`) to isolate
     the cardiac frequency band.
  3. Apply a short moving-average smoother (200 ms by default).
  4. Detect peaks via an adaptive rolling-quantile threshold.
  5. Compute instantaneous BPM from inter-beat intervals (IBI).
  6. Replace physiologically implausible IBIs with local averages.

Returns a DataFrame with columns ``'timestamp'`` (datetime) and ``'BPM'``
(float), one row per detected inter-beat interval.
"""
import numpy as np
import pandas as pd
...
# Bandpass filter cutoff frequencies for the PPG cardiac band.
PPG_BANDPASS_LOW_HZ = 0.5
PPG_BANDPASS_HIGH_HZ = 10.0
...
def calculate_bpm_from_ppg(df,
    ...
    # 3. Bandpass filter using module-level constants, with zero-phase
    nyq = fs / 2.0
    low = PPG_BANDPASS_LOW_HZ / nyq
    high = min(PPG_BANDPASS_HIGH_HZ, nyq*0.99) / nyq
    ...
    if debug:
        ...
        plt.xlabel('Time (s)'); plt.ylabel('PPG (mV)')
        plt.title(f'Bandpass Filtered ({PPG_BANDPASS_LOW_HZ}–{PPG_BANDPASS_HIGH_HZ} Hz)')
    ...
    typical_difference = np.median(ibi)
    masked_in = (ibi <= 1.5*typical_difference)
    # Compute instantaneous BPM
    hr_inst = 60.0 / ibi

    # Create full BPM array with NaNs where data is masked out
    hr_inst_full = np.full_like(hr_inst, np.nan, dtype=np.float64)
    hr_inst_full[masked_in] = hr_inst[masked_in]

    # Fill NaNs using average of previous 3 and next 3 valid values
    hr_series = pd.Series(hr_inst_full)
    for idx in hr_series[hr_series.isna()].index:
        start = max(0, idx - 3)
        end = idx + 4  # idx + 3 + 1, since end index is exclusive
        window_vals = hr_series[start:end].dropna()
        if len(window_vals) >= 3:  # At least 3 valid values in the window
            hr_series.iloc[idx] = window_vals.mean()

    # Timestamps for instantaneous HRs
    inst_times = peak_times[1:]
    inst_timestamps = [base_ts + pd.to_timedelta(t, unit='s') for t in inst_times]

    # Create DataFrame
    ib_df = pd.DataFrame({'timestamp': inst_timestamps, 'BPM': hr_series.values})

    #hr_inst = np.zeros_like

    ib_df = pd.DataFrame({'timestamp': inst_timestamps, 'BPM': hr_inst_full})
    ...
    if debug and not ib_df.empty:
        ...
        plt.figure(figsize=(10, 3))
        plt.plot(np.array(inst_timestamps), ib_df['BPM'], marker='o', linestyle='-')
        plt.xlabel('Time'); plt.ylabel('BPM')
        plt.title('Instantaneous Heart Rate'); plt.tight_layout(); plt.show()
