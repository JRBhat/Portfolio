...
def update_missing_values(identifier, idtype, row_dict):
    ...
    if data_chemspider:
        ...
        if data_chemspider.get("English Name"):
            if row_dict["IUPAC"] != data_chemspider.get("English Name"):
                row_dict["IUPAC"] = data_chemspider.get("English Name")
    else:
...
if __name__ == "__main__":
    # main()
    # cas = "108-94-1"
    iupac = "(2S)-1-[(2-Fluorophenyl)amino]-1-oxo-2-propanyl 2,3-dihydro-1,4-benzodioxine-6-carboxylate"
    smiles = "C[C@H](OC(=O)c1ccc2c(c1)OCCO2)C(=O)Nc1ccccc1F"
    ...
    """
    TODO: Lab_report -> Invalid CAS --> IUPAC_Name -> PUBchem = SMILES - compare PUBCHEM SMILES with existing SMILES -> if found then use that Substance row for the final report
    """
