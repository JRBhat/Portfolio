...
def admin_dashboard(db: Session = Depends(get_db)):
        ...
        <tr>
            ...
            <td>
                <form method="post" action="/admin/device/delete/{d.id}" style="display: inline;">
                    <button type="submit" class="delete-btn" onclick="return confirm('Delete this device and all associated training records?')">Delete</button>
                </form>
            </td>
        </tr>
        """
    ...
    for t in technicians:
        training_count = len(t.trainings)
        tech_rows += f"""
        <tr>
            <td>{t.name}</td>
            <td>{training_count}</td>
            <td>
                <form method="post" action="/admin/technician/delete/{t.id}" style="display: inline;">
                    <button type="submit" class="delete-btn" onclick="return confirm('Delete this technician and all their training records?')">Delete</button>
                </form>
            </td>
        </tr>
        """
    ...
    <div class="nav">
        ...
        <a href="/admin/manage-technicians">Manage Technicians</a>
    </div>
    ...
    <h2 style="margin-top: 30px;">Registered Technicians</h2>
    <table>
        <tr>
        ...
        {tech_rows if tech_rows else '<tr><td colspan="3" style="text-align: center;">No technicians registered</td></tr>'}
    </table>
    """
    return HTML_BASE.format(content=content)
...
def delete_device(device_id: int, db: Session = Depends(get_db)):
    device = db.query(Device).filter(Device.id == device_id).first()
    if device:
        # Delete all training records associated with this device
        db.query(Training).filter(Training.device_id == device_id).delete()
        db.delete(device)
        db.commit()
    return RedirectResponse(url="/admin/dashboard", status_code=303)

@app.post("/admin/technician/delete/{tech_id}")
def delete_technician(tech_id: int, db: Session = Depends(get_db)):
    tech = db.query(Technician).filter(Technician.id == tech_id).first()
    if tech:
        # Delete all training records associated with this technician
        db.query(Training).filter(Training.technician_id == tech_id).delete()
        db.delete(tech)
        db.commit()
    return RedirectResponse(url="/admin/dashboard", status_code=303)
...
def all_trainings(db: Session = Depends(get_db)):
    ...
    for t in trainings:
        ...
        <tr>
            ...
            <td class="{status_class}">{status_text}</td>
            <td>
                <form method="post" action="/admin/training/delete/{t.id}" style="display: inline;">
                    <button type="submit" class="delete-btn" onclick="return confirm('Delete this training record?')">Delete</button>
                </form>
            </td>
        </tr>
        """
    ...
    <table>
        ...
        {rows if rows else '<tr><td colspan="6" style="text-align: center;">No training records</td></tr>'}
    </table>
    """
    return HTML_BASE.format(content=content)

@app.post("/admin/training/delete/{training_id}")
def delete_training(training_id: int, db: Session = Depends(get_db)):
    training = db.query(Training).filter(Training.id == training_id).first()
    if training:
        db.delete(training)
        db.commit()
    return RedirectResponse(url="/admin/all-trainings", status_code=303)
